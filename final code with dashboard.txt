...
class DataConsolidator:
    ...
    def consolidate(data: List[Dict]) -> List[Dict]:
        ...
        - Skill name + rating + experience → Single "Skill" row
        - Education degree + institution + year → Single "Education" row
        - Certification name + score + year → Single "Certification" row
        """
        print("\n🔧 Consolidating related fields into single rows...")
        ...
        print(f"✅ Consolidated: {len(data)} rows → {len(consolidated)} rows (reduced by {len(data) - len(consolidated)})")
        return consolidated
...
Return ONLY the JSON array:"""
        ...
        for attempt in range(max_retries):
            try:
                print(f"🤖 Extracting data (Attempt {attempt + 1}/{max_retries})...")
...
def main():
    ...
    consolidated_data = consolidator.consolidate(structured)

    # Save Excel
    print("\nSaving Excel file...")
    output = excel_generator.create_excel(consolidated_data)
